...
def lancer_ocr(pdf_path, psm, oem, preprocess=True):
...
def main():
    ...
    preprocess_pdf = config.get("preprocess_pdf", False)
    preprocess_tables = config.get("preprocess_tables", False)
    crop_tables = config.get("crop_tables", False)

    # Paramètres OCR pour PDF complet
    ocr_pdf_config = config.get("ocr", {}).get("pdf", {})
    psm_pdf = ocr_pdf_config.get("psm", 3)
    oem_pdf = ocr_pdf_config.get("oem", 3)

    # Paramètres OCR pour tableaux
    ...
    lancer_ocr(args.pdf, psm=psm_pdf, oem=oem_pdf, preprocess=preprocess_pdf)

    if crop_tables:
        print("✂️ Découpage des tableaux...")
        lancer_crop_tables(args.pdf, preprocess=preprocess_pdf)
        ...
        for tab_path in tableaux:
            ...
            lancer_ocr(tab_path, psm=psm_tables, oem=oem_tables, preprocess=preprocess_tables)
